-- Pending bookings are a queue (Jonossa), not a hard hold.
-- create_tool_booking_public only blocks on loans + confirmed bookings so
-- multiple guests can join the queue; owner confirms FIFO from pending list.
...
) IS 'Julkinen varauspyyntö (pending = jonossa). Estää vain lainat + confirmed; useampi pending sallittu samaan ikkunaan.';

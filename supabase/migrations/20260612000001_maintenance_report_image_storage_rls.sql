-- Storage-read: sama näkyvyys kuin maintenance_report_images -taulussa.
...
BEGIN
  ...
  RETURN EXISTS (
    SELECT 1
    FROM maintenance_report_images mri
    JOIN maintenance_reports mr ON mr.id = mri.maintenance_report_id
    WHERE mri.storage_path = object_name

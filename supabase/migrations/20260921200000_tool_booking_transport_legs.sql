-- Four transport legs for tool bookings:
-- none = self both ways; pickup = self outbound + company return; delivery = company outbound + self return; both = company both ways.
-- Fee = per-leg fee × number of company legs. Distance/address required for any company leg.

ALTER TABLE tool_bookings
  DROP CONSTRAINT IF EXISTS tool_bookings_delivery_mode_check;

ALTER TABLE tool_bookings
  ADD CONSTRAINT tool_bookings_delivery_mode_check
  CHECK (delivery_mode IN ('none', 'delivery', 'pickup', 'both'));

COMMENT ON COLUMN tool_bookings.delivery_mode IS
  'none=Haen ja palautan itse; pickup=Haen työkalu(t) itse, tilaan palautuksen; delivery=Työkalujen haku tilataan, palautan itse; both=Tilaan kuljetuksen molempiin suuntiin';

COMMENT ON COLUMN companies.delivery_enabled IS 'Tarjotaanko vienti (toimitus vuokraajalle) työkaluvarauksille.';
COMMENT ON COLUMN companies.pickup_enabled IS 'Tarjotaanko palautusnouto (nouto vuokraajalta) työkaluvarauksille.';
COMMENT ON COLUMN companies.delivery_min_fee_eur IS 'Yhden kuljetusosuuden minimihinta (lyhyt matka / raja-km asti).';
COMMENT ON COLUMN companies.delivery_distance_limit_km IS 'Raja-km per osuus: tähän asti minimihinta, yli → + €/km.';
COMMENT ON COLUMN companies.delivery_per_km_eur IS 'Yli raja-km:n osuuden €/km (per kuljetusosuus).';
...
) IS 'Julkinen varauspyyntö (pending = jonossa). Kuljetus: none|delivery|pickup|both; maksu = osuusmaksu × osuuksien määrä.';

...
export default function WorkReportSectionDialog({ open, title, onClose, children, wide = false }: Props) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="leave-draft-overlay" role="presentation" onClick={onClose}>
      <div
        className={`leave-draft-dialog work-report-section-dialog panel${wide ? ' work-report-section-dialog--wide' : ''}`}
        role="dialog"
        aria-modal="true"

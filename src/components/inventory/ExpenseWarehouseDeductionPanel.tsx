import { formatExpenseWarehouseCostLabel, isExpenseWarehouseCostLine } from '../../lib/expenseWarehouseDeduction';
...
function collectRows(logs: WorkReportDailyLog[], warehouseCompanyId: string): Row[] {
  ...
  for (const log of logs) {
    for (const line of log.expense_lines ?? []) {
      if (!isExpenseWarehouseCostLine(line, warehouseCompanyId)) continue;
...
export default function ExpenseWarehouseDeductionPanel({
  logs,
  warehouseCompanyId,
  canEdit,
  busyLineId,
        ...
        <table className="data-table">
          ...
          <tbody>
            {rows.map(({ line, logDate }) => {
              ...
              const total = Math.round(Number(line.qty) * unit * 100) / 100;
              return (
                <tr key={line.id}>
                  <td>{new Date(`${logDate}T12:00:00`).toLocaleDateString('fi-FI')}</td>
                  <td>{formatExpenseWarehouseCostLabel(line, Boolean(line.warehouse_cost_deducted))}</td>
